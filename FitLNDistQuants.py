...
def FitLNDistQuants(
    target_mean_val: float,
    target_quant_vals: float,
    minval: float,
    quant_levs: float,
) -> float:
    ...
    Args:
        target_mean_val (float): The initial guess of the distribution mean
        target_quant_vals (float): The target values for the fitted the 'quant_levs'.
        minval (float): A shift in the mean used in the initial guess
        quant_levs (float): A list of the desired quantile levels for the fitting process

    Return:
        A list of the estimated log-normal parameters as returned from a scipy
    ...
    def mintarget(thet):
        return np.sum(
            (targetquants(thet[0], thet[1], thet[2]) - target_quant_vals) ** 2
        )
    ...
    optim_results = minimize(
        mintarget, method="L-BFGS-B", x0=init_guess, bounds=my_bnds
    )
