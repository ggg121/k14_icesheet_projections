...
def ProjectGSL(baaccelsamps, araccelsamps, islastdecade, targyears):
    """Produce global sea-level change projections

    This function produces global sea-level change projections
    after sampling the fitted ice sheet distributions. Projections
    are made by convoluting both the BA and AR estimates.

    Args:
        baaccelsamps (float): BA samples generated by SampleISDists
        araccelsamps (float): AR samples generated by SampleISDists
        islastdecade (float): Ice sheet melt accelerations for the last decade
        targyears (int): Years to produce a global sea-level change projection

    Return:
        An array of dimensions [nsamps, len(targyears), 3] of samples of
        sea-level change contributions due to ice sheets.
    ...
    # Note: "targyears - 1999" is used instead of "targyears - 2000"
    # to avoid divide-by-zero problems
    def proj_over_samp(samp, targyears, islastdecade):
        return (islastdecade * (targyears - 1999)) + (
            0.5 * samp * (targyears - 2011) ** 2 * (targyears > 2011)
        )

    def add_last_two(x):
        return x[-1] + x[-2]

    basamps = np.apply_along_axis(
        proj_over_samp, 1, baaccelsamps, targyears=targyears, islastdecade=islastdecade
    )
    arsamps = np.apply_along_axis(
        proj_over_samp,
        1,
        araccelsamps,
        targyears=targyears,
        islastdecade=np.array([islastdecade[0], islastdecade[1] + islastdecade[2]]),
    )

    # Append the total AIS contribution to the end of 'basamps'
    basamps = np.concatenate(
        (basamps, np.apply_along_axis(add_last_two, 2, basamps)[:, :, None]), 2
    )
    ...
    # Apply constraint to GIS and WAIS
    ...
    arsamps = np.stack(
        (arsamps[:, :, 0], hysamps_wais, arsamps[:, :, 1] - hysamps_wais), 2
    )
