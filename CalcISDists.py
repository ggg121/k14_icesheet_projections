...
def CalcISDists(
    barates2100: float, lastdecadegt: float, aris2090: float
) -> tuple[float, float, float, float, float, float]:
    """Fit log-normal distributions to the ice sheet melt estimates

    Args:
        barates2100 (float): BA13 ice sheet melt rates in 2100
        lastdecadegt (float): Ice sheet mass loss in giga-tonnes
        aris2090 (float): IPCC AR5 ice sheet contributions to SLR in 2090

    Return:
        A tuple of the various estimated distribution parameters...
    ...
    batheteais = fitLN(
        barates2100[2, 1], barates2100[2, 2:5], barates2100[2, 0], [0.05, 0.5, 0.95]
    )
    bathetwais = fitLN(
        barates2100[1, 1], barates2100[1, 2:5], barates2100[1, 0], [0.05, 0.5, 0.95]
    )
    bathetgis = fitLN(
        barates2100[0, 1], barates2100[0, 2:5], barates2100[0, 0], [0.05, 0.5, 0.95]
    )
